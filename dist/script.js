...
window.addEventListener('DOMContentLoaded', function () {
  ...
  player.init();
  var playerSecond = new _modules_playVideo__WEBPACK_IMPORTED_MODULE_0__["default"]('.module__video-item .play', '.overlay');
  playerSecond.init();
...
/*#__PURE__*/
function () {
  function Difference(container, items) {
  ...
  _createClass(Difference, [{
    ...
    value: function bindTriggers() {
      ...
      this.container.querySelector('.plus').addEventListener('click', function () {
        if (_this.counter !== _this.items.length - 2) {
          ...
          _this.items[_this.items.length - 1].classList.add('animated', 'fadeOut');
...
function () {
  ...
  _createClass(Form, [{
    ...
    key: "checkDateInputs",
    value: function checkDateInputs() {
      var dateInputs = document.querySelectorAll('[type="date"]');
      dateInputs.forEach(function (input) {
        input.addEventListener('input', function () {
          if (+input.value.slice(0, -6) >= 9999) {
            input.value = input.value.slice(-10, -6) + input.value.slice(-6);
          }
        });
      });
    }
